package space.marsden.mods.chaoscore.items.oc.traits
...
trait Delegate {
  ...
  protected def tooltipCosts(stack: ItemStack, tooltip: java.util.List[String]) {
    if (stack.hasTagCompound && stack.getTagCompound.hasKey("asc:data")) {
      val data = stack.getTagCompound.getCompoundTag("asc:data")
