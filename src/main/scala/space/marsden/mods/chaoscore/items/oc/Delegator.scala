package space.marsden.mods.chaoscore.items.oc
...
import net.minecraft.item.{EnumAction, EnumRarity, Item, ItemStack}
...
class Delegator(prefix: String = "ascension") extends Item with driver.item.UpgradeRenderer with Chargeable {
  setHasSubtypes(true)
  setCreativeTab(Ascension.tab) // todo: do not do this!
  ...
  val subItems: ArrayBuffer[Delegate] = mutable.ArrayBuffer.empty[Delegate]

  def add(subItem: Delegate): Int = {
  ...
  override def getUnlocalizedName(stack: ItemStack): String =
    Delegator.subItem(stack) match {
      case Some(subItem) => "item." + prefix + "." + subItem.unlocalizedName
