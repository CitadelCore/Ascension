...
class CardCoprocessor(val parent: Delegator, val tier: Int) extends Delegate {
  ...
  private object HelperInventory extends CoprocessorInventory {
  ...
  override protected def tooltipExtended(stack: ItemStack, tooltip: util.List[String]) {
    ...
    if (KeyBindings.showExtendedTooltips) {
      ...
      if (stacks.nonEmpty) {
        tooltip.addAll(Tooltip.get("coprocessor.Components"))
